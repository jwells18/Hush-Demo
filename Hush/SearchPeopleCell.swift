...
class SearchPeopleCell: UITableViewCell{
    ...
    func configure(user: User?){
        //Set ImageView Image
        if user?.image != nil{
            mainImageView.sd_setImage(with: URL(string: (user?.image)!), placeholderImage: UIImage(named: "hushIcon"))
        }
        else{
            mainImageView.image = UIImage(named: "hushIcon")
        }
        mainTitleLabel.text = user?.name
        mainSubTitleLabel.text = user?.bio
    }
}
